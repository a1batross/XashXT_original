...
static glsl_program_t *GL_CreateUberShader( GLuint slot, const char *glname, const char *options = NULL, pfnShaderCallback pfnInitUniforms = NULL )
{
	...
	ALERT( at_console, "CompileUberShader #%i: %s\n%s\n", slot, glname, options );
...
word GL_UberShaderForSolidBmodel( msurface_t *s, bool translucent )
{
	...
	if( es->shaderNum && es->glsl_sequence == tr.glsl_valid_sequence )
		return es->shaderNum; // valid
	...
	if( mirrorSurface || portalSurface )
...
#endif
	es->glsl_sequence = tr.glsl_valid_sequence;
	ClearBits( s->flags, SURF_NODRAW );
	es->shaderNum = shaderNum;
...
{
	if( g->shaderNum && g->glsl_sequence == tr.glsl_valid_sequence )
		return g->shaderNum; // valid
	...
	g->glsl_sequence = tr.glsl_valid_sequence;
...
word GL_UberShaderForGrassDlight( plight_t *pl, struct grass_s *g )
{
	...
	if( !shader )
	{
		SetBits( g->flags, FGRASS_NODLIGHT );
		return 0; // something bad happens
	}
...
#if 0
	// prepare sunshafts
	glsl.genSunShafts = shader = GL_InitGPUShader( "HW_GenSunShafts", "generic", "genshafts" );
	GL_InitGenShaftsUniforms( shader );

	// render sunshafts
	glsl.drawSunShafts = shader = GL_InitGPUShader( "HW_DrawSunShafts", "generic", "drawshafts" );
	GL_InitDrawShaftsUniforms( shader );

	// draw sun & skybox
	glsl.skyboxEnv[0] = shader = GL_InitGPUShader( "HW_SkyBoxGeneric", "generic", "skybox" );
	GL_InitSkyBoxUniforms( shader );

	// draw sun & skybox
	glsl.skyboxEnv[1] = shader = GL_InitGPUShader( "HW_SkyBoxDaytime", "generic", "skybox", "#define SKYBOX_DAYTIME\n" );
	GL_InitSkyBoxUniforms( shader );
#endif
	// studio shadowing
